...
const EXT_ID = "com.safelight.advanced-library-sort";
...
for (const pluginsDir of resolvePluginsDirs()) {
  ...
  console.log(`[advanced-library-sort] installed to ${target}`);
}
console.log("[advanced-library-sort] restart SafeLight (or re-enable the extension) to load it.");
